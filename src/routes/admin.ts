...
import { requireAuth, requirePerm, authorizeBranch, branchFilter, getPrincipal } from '../lib/authz.ts';
...
import { adjustPoints, listLedger, getBalance, expireLots } from '../domain/points.ts';
import { recordTransaction } from '../domain/transactions.ts';
import { listBranches, upsertBranch } from '../domain/branches.ts';
import { createCoupon, listCoupons, type CouponType } from '../domain/coupons.ts';
import {
  adminOverview,
  topCustomers,
  salesByBranch,
  ...
  birthdayMembers,
  birthMonthMembers,
} from '../domain/segments.ts';
import { flushOutbox } from '../domain/notifications.ts';
import { setSetting, getLoyaltyConfig, loyaltyApprovalStatus, LOYALTY_KEYS } from '../domain/settings.ts';
import { listPolicies, setPolicy, setPolicyApproved, getPolicyInt } from '../domain/policy.ts';
import { listReferrals, referralSummary } from '../domain/referral.ts';
import { listTiers, upsertTier, deleteTier, upgradeMembership, listMemberPurchases, recomputeTier } from '../domain/membership.ts';
import { getStoreProfile, setStoreProfile, getTheme, setTheme } from '../domain/store.ts';
import { getSignupFields, setSignupFields, type SignupField } from '../domain/signup.ts';
import { createPosKey, listPosKeys, revokePosKey } from '../domain/pos.ts';
import { listTags, createTag, deleteTag, addMemberTag, removeMemberTag, memberTags } from '../domain/tags.ts';
import { listClaims, getClaim, getClaimImage, approveClaim, rejectClaim } from '../domain/receipts.ts';
import { importFoodStoryCsv, matchAllClaims, topProducts, salesByCategory, memberItems, listBranchAliases, upsertBranchAlias, recordImport, listImports } from '../domain/pos_import.ts';
import { exportMemberData, setConsent, consentLog, anonymizeMember, anonymizeInactive } from '../domain/pdpa.ts';
import { previewAudience, createCampaign, listCampaigns, requireCampaign, updateCampaign, cancelCampaign, sendCampaign, campaignDeliveries, dispatchDueCampaigns, type AudienceFilter } from '../domain/campaigns.ts';
import { TEMPLATES, LIFF_SECTIONS, createRichMenu, listRichMenus, requireRichMenu, updateRichMenu, setRichMenuImage, getRichMenuImage, publishRichMenu, setDefaultRichMenu, deleteRichMenu, type ButtonInput } from '../domain/richmenu.ts';
...
function requireAllBranchReports(ctx: Ctx): void {
...
export function registerAdminRoutes(router: Router): void {
  ...
  // --- Reports (all-branch) ------------------------------------------------
  router.get('/api/admin/overview', (ctx) => { requireAllBranchReports(ctx); return adminOverview(); },
    [...auth, requirePerm('reports.read')]);
  router.get('/api/admin/top-customers', (ctx) => { requireAllBranchReports(ctx); return { customers: topCustomers(Number(ctx.query.get('limit') ?? 10)) }; },
    [...auth, requirePerm('reports.read')]);
  router.get('/api/admin/sales-by-branch', (ctx) => { requireAllBranchReports(ctx); return { branches: salesByBranch() }; },
    [...auth, requirePerm('reports.read')]);
  router.get('/api/admin/birthdays', (ctx) => {
    const mmdd = ctx.query.get('mmdd');
    if (mmdd) return { members: birthdayMembers(mmdd) };
  ...
  router.post(
    '/api/admin/members/:id/points/adjust',
    (ctx) => {
      const m = requireMember(intParam(ctx, 'id'));
  ...
  router.post(
    ...
    (ctx) => {
      ...
      const coupon = createCoupon({
        ...
        type: requireString(b, 'type') as CouponType,
        value: requireInt(b, 'value'),
        config: (b['config'] as Record<string, unknown> | undefined) ?? undefined,
        branchScope: (b['branchScope'] as string[] | undefined) ?? null,
  ...
  router.patch(
    ...
    (ctx) => {
      const b = body(ctx);
      const updates = (b['updates'] as Record<string, unknown> | undefined) ?? b;
      const applied: string[] = [];
      for (const [key, value] of Object.entries(updates)) {
        if (key === 'updates' || key === 'approvals') continue;
        setPolicy(key, value);
        applied.push(key);
      }
      const approvals = (b['approvals'] as Record<string, boolean> | undefined) ?? {};
      const approved: string[] = [];
      for (const [key, ok] of Object.entries(approvals)) {
        setPolicyApproved(key, !!ok);
        approved.push(`${key}=${ok ? 'yes' : 'no'}`);
      }
      audit({ actor: getPrincipal(ctx), action: 'settings.update', metadata: { keys: applied, approvals: approved }, ip: clientIp(ctx) });
  ...
  router.patch(
    ...
    (ctx) => {
      ...
      if (earn !== undefined) setSetting(LOYALTY_KEYS.earnBahtPerPoint, String(earn));
      if (redeem !== undefined) setSetting(LOYALTY_KEYS.redeemBahtPerPoint, String(redeem));
      if (expiry !== undefined) setSetting(LOYALTY_KEYS.expiryDays, String(expiry));
      if (approved !== undefined) setSetting(LOYALTY_KEYS.rulesApproved, approved ? 'true' : 'false');
  ...
  router.patch(
    '/api/admin/signup-fields',
    (ctx) => {
      const b = body(ctx);
      const fields = setSignupFields((b['fields'] as SignupField[]) ?? []);
  ...
  router.delete(
    '/api/admin/members/:id/tags/:tagId',
    (ctx) => {
      const m = requireMember(intParam(ctx, 'id'));
      authorizeBranch(ctx, 'members.write', m.home_branch_id ?? undefined);
  ...
  router.post(
    '/api/admin/pos-import',
    (ctx) => {
      const csv = typeof ctx.body === 'string' ? ctx.body : optionalString(body(ctx), 'csv');
      if (!csv) throw notFound('csv is required (send the CSV as text or {"csv":"..."})');
  ...
  router.get(
    '/api/admin/receipts/:id/image',
    (ctx) => {
      const claim = getClaim(intParam(ctx, 'id'));
      if (!claim) throw notFound('receipt not found');
  ...
  router.post('/api/admin/campaigns/preview', (ctx) => {
    const b = body(ctx);
    return previewAudience((b['audience'] as AudienceFilter) ?? {});
  }, [...auth, requirePerm('campaigns.read')]);
  ...
  router.post('/api/admin/campaigns', (ctx) => {
    ...
    const c = createCampaign({
      ...
      messages: b["messages"] as LineMessage[] | undefined,
      audience: (b['audience'] as AudienceFilter) ?? {},
  ...
  router.patch('/api/admin/campaigns/:id', (ctx) => {
    ...
    const c = updateCampaign(id, {
      ...
      text: optionalString(b, 'text'),
      messages: b["messages"] as LineMessage[] | undefined,
      audience: b['audience'] as AudienceFilter | undefined,
  ...
  router.post('/api/admin/richmenus', (ctx) => {
    ...
    const r = createRichMenu({
      ...
      buttons: (b['buttons'] as ButtonInput[]) ?? [],
    }, getPrincipal(ctx));
  ...
  router.patch('/api/admin/richmenus/:id', (ctx) => {
    ...
    const r = updateRichMenu(id, {
      ...
      buttons: b['buttons'] as ButtonInput[] | undefined,
    });
  ...
  router.post('/api/admin/richmenus/:id/image', (ctx) => {
    ...
    if (!m) throw forbidden('imageDataUrl (data:image/png|jpeg;base64,...) is required');
