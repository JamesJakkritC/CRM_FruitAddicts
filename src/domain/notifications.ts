...
export function enqueue(
  ...
  const res = db
    .prepare(
      `INSERT INTO line_outbox(member_id, to_line_user_id, kind, payload_json, status, dedup_key, created_at)
       VALUES(?,?,?,?, 'pending', ?, ?)
...
export async function flushOutbox(
  ...
  for (const row of rows) {
    ...
    try {
      await provider.pushMessage(row.to_line_user_id, JSON.parse(row.payload_json) as LineMessage[]);
      ...
      db.prepare(`UPDATE line_outbox SET status=?, error=?, attempts=? WHERE id=?`).run(
        parked ? 'failed' : 'pending',
