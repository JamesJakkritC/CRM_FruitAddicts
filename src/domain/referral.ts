...
import { getPolicyBool, getPolicyJson } from './policy.ts';
...
export interface ReferralRewardConfig {
  ...
  minFirstPurchaseSatang: number;
}

export interface Referral {
  id: number;
  referrer_member_id: number;
  referred_member_id: number;
  code: string;
  status: string;
  qualifying_transaction_id: number | null;
...
function genCode(memberId: number): string {
...
export function getOrCreateReferralCode(memberId: number): { code: string; deepLink: string } {
  return tx((db) => {
    requireMember(memberId, db);
    let row = asRow<{ code: string }>(
      db.prepare('SELECT code FROM referral_codes WHERE member_id = ?').get(memberId),
    );
    if (!row) {
...
export function getReferralByCode(code: string, db: DatabaseSync = getDb()): { member_id: number } | undefined {
  return asRow<{ member_id: number }>(
    db.prepare('SELECT member_id FROM referral_codes WHERE code = ?').get(code),
  );
}
 ...
 */
export function recordReferral(input: { code: string; referredMemberId: number }): Referral {
  return tx((db) => {
    if (!getPolicyBool('referral.enabled', db)) throw unprocessable('referral program is disabled');
    ...
    if (owner.member_id === input.referredMemberId) throw badRequest('self-referral is not allowed');

    const existing = asRow<Referral>(
      db.prepare('SELECT * FROM referrals WHERE referred_member_id = ?').get(input.referredMemberId),
    );
    if (existing) {
      if (existing.referrer_member_id === owner.member_id) return existing; // idempotent
      throw conflict('member has already been referred by someone else');
    }

    const info = db
      .prepare(
        `INSERT INTO referrals(referrer_member_id, referred_member_id, code, status, created_at)
         VALUES(?,?,?, 'pending', ?)`,
      )
      .run(owner.member_id, input.referredMemberId, input.code, now());
    return asRow<Referral>(db.prepare('SELECT * FROM referrals WHERE id = ?').get(Number(info.lastInsertRowid)))!;
  });
}
 ...
 */
export function maybeRewardReferralOnPurchase(
  db: DatabaseSync,
  args: { referredMemberId: number; transactionId: number; netAmount: number },
): { rewarded: boolean } {
  if (!getPolicyBool('referral.enabled', db)) return { rewarded: false };
  const ref = asRow<Referral>(
    db.prepare("SELECT * FROM referrals WHERE referred_member_id = ? AND status = 'pending'").get(args.referredMemberId),
  );
  if (!ref) return { rewarded: false };

  const cfg = getPolicyJson<ReferralRewardConfig>('referral.reward', db);
  if (args.netAmount < (cfg.minFirstPurchaseSatang ?? 0)) return { rewarded: false }; // not qualified yet
  ...
  if (flip.changes === 0) return { rewarded: false };
...
export function referralSummary(memberId: number, db: DatabaseSync = getDb()): ReferralSummary {
  const code = asRow<{ code: string }>(db.prepare('SELECT code FROM referral_codes WHERE member_id = ?').get(memberId));
  const rows = asRows<{ status: string }>(
    db.prepare('SELECT status FROM referrals WHERE referrer_member_id = ?').all(memberId),
  );
  return {
    code: code?.code ?? null,
...
export function listReferrals(limit = 100, db: DatabaseSync = getDb()): Referral[] {
  return asRows<Referral>(db.prepare('SELECT * FROM referrals ORDER BY id DESC LIMIT ?').all(Math.min(Math.max(limit, 1), 500)));
}
