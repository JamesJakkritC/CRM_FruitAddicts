...
import { getClaim, approveClaim } from "./receipts.js";
...
// --- Branch aliases --------------------------------------------------------
export function upsertBranchAlias(alias, branchId, db = getDb()) {
    const a = alias.trim();
...
export function listBranchAliases(db = getDb()) {
    return asRows(db.prepare('SELECT alias, branch_id FROM branch_aliases ORDER BY alias').all());
}
function resolveBranch(raw, db) {
    if (!raw)
        return null;
    const row = asRow(db.prepare('SELECT branch_id FROM branch_aliases WHERE alias = ?').get(raw.trim()));
    return row?.branch_id ?? null;
}
export function importFoodStoryCsv(text, sourceFile, db = getDb()) {
    const rows = parseCsv(text);
    const headerRow = rows.find((r) => r.some((c) => c.trim() === COL.code));
    if (!headerRow)
        throw badRequest('CSV header not found (expected a "หมายเลขใบเสร็จ / ID" column) — is this a FoodStory sale-by-bill-detail export?');
    const H = headerIndex(headerRow);
    const need = [COL.code, COL.name, COL.net, COL.branch, COL.date];
    for (const n of need)
        if (H[n] === undefined)
            throw badRequest(`CSV missing column: ${n}`);
    ...
    tx((d) => {
        for (const bill of bills.values()) {
            ...
            if (!branchId && bill.rawBranch)
                unmapped.add(bill.rawBranch);
            const info = d
                .prepare(`INSERT OR IGNORE INTO pos_bills(receipt_code, raw_branch, branch_id, bill_date, bill_time, channel, payment_type, total_satang, item_count, source_file, created_at)
           VALUES(?,?,?,?,?,?,?,?,?,?,?)`)
...
export function recordImport(summary, filename, actor, db = getDb()) {
    db.prepare(`INSERT INTO pos_imports(filename, bills_imported, bills_skipped, items_imported, claims_approved, unmapped_json, actor_user_id, actor_username, created_at)
     VALUES(?,?,?,?,?,?,?,?,?)`).run(filename, summary.billsImported, summary.billsSkipped, summary.itemsImported, summary.claimsApproved, JSON.stringify(summary.unmappedBranches), actor?.userId ?? null, actor?.username ?? null, now());
}
export function listImports(limit = 50, db = getDb()) {
...
function findBillForClaim(claim, db) {
    if (!claim.receipt_code)
        return undefined;
    const exact = asRow(db.prepare('SELECT id, total_satang, branch_id FROM pos_bills WHERE receipt_code=? AND branch_id=? AND bill_date=? AND matched_claim_id IS NULL').get(claim.receipt_code, claim.branch_id, claim.receipt_date));
    if (exact)
        return exact;
    return asRow(db.prepare('SELECT id, total_satang, branch_id FROM pos_bills WHERE receipt_code=? AND branch_id=? AND matched_claim_id IS NULL ORDER BY id LIMIT 1').get(claim.receipt_code, claim.branch_id));
}
...
export function matchClaim(claimId, db = getDb()) {
    const claim = getClaim(claimId, db);
    if (!claim || !claim.receipt_code)
...
export function matchAllClaims(db = getDb()) {
    const claims = asRows(db.prepare("SELECT id FROM receipt_claims WHERE receipt_code IS NOT NULL AND status IN ('pending','approved') AND id NOT IN (SELECT matched_claim_id FROM pos_bills WHERE matched_claim_id IS NOT NULL)").all());
    let approved = 0;
    let linked = 0;
...
// --- Reports ---------------------------------------------------------------
export function topProducts(limit = 20, db = getDb()) {
    return db
        .prepare(`SELECT name, category, SUM(qty) AS qty, SUM(net_satang) AS sales_satang, COUNT(DISTINCT bill_id) AS bills
...
export function salesByCategory(db = getDb()) {
    return db
        .prepare(`SELECT COALESCE(NULLIF(category,''),'(ไม่ระบุ)') AS category, SUM(qty) AS qty, SUM(net_satang) AS sales_satang
...
export function memberItems(memberId, limit = 100, db = getDb()) {
    return db
        .prepare(`SELECT b.bill_date, b.receipt_code, i.name, i.qty, i.net_satang, i.category
...
//# sourceMappingURL=pos_import.js.map