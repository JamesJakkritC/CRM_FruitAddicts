...
export function getBalance(memberId, db = getDb()) {
    const row = db
        .prepare(`SELECT COALESCE(SUM(remaining), 0) AS bal
         FROM point_lots
...
function writeLedger(db, memberId, delta, type, ref = {}) {
    const balanceAfter = getBalance(memberId, db);
    const info = db
        .prepare(`INSERT INTO point_ledger(member_id, delta, type, balance_after, ref_type, ref_id, note, created_at)
       VALUES(?,?,?,?,?,?,?,?)`)
        .run(memberId, delta, type, balanceAfter, ref.type ?? null, ref.id ?? null, ref.note ?? null, now());
    return asRow(db.prepare('SELECT * FROM point_ledger WHERE id = ?').get(Number(info.lastInsertRowid)));
}
/**
 ...
 */
export function earn(db, memberId, points, opts = {}) {
    if (!Number.isInteger(points) || points <= 0)
        throw badRequest('points to earn must be a positive integer');
    const ts = now();
    const expiryDays = opts.expiryDays ?? getLoyaltyConfig(db).expiryDays;
    const expiresAt = addDays(ts, expiryDays);
    db.prepare(`INSERT INTO point_lots(member_id, source_txn_id, amount, remaining, earned_at, expires_at, status, created_at)
     VALUES(?,?,?,?,?,?, 'active', ?)`).run(memberId, opts.sourceTxnId ?? null, points, points, ts, expiresAt, ts);
...
function consume(db, memberId, points) {
    let left = points;
    const lots = db
        .prepare(`SELECT id, remaining FROM point_lots
         WHERE member_id = ? AND status = 'active' AND remaining > 0
           AND (expires_at IS NULL OR expires_at > ?)
    ...
    for (const lot of lots) {
        ...
        db.prepare(`UPDATE point_lots SET remaining = ?, status = CASE WHEN ? = 0 THEN 'consumed' ELSE status END
         WHERE id = ?`).run(newRemaining, newRemaining, lot.id);
        left -= take;
    }
    if (left > 0) {
...
export function adjustPoints(args) {
    ...
    const { result } = withIdempotency('adjust', args.idempotencyKey, args, (db) => {
        ...
        if (args.delta > 0) {
            ...
            db.prepare(`INSERT INTO point_lots(member_id, source_txn_id, amount, remaining, earned_at, expires_at, status, created_at)
         VALUES(?, NULL, ?, ?, ?, ?, 'active', ?)`).run(args.memberId, args.delta, args.delta, ts, addDays(ts, expiryDays), ts);
 ...
 */
export function expireLots(asOf = now()) {
    return tx((db) => {
        const rows = db
            .prepare(`SELECT member_id, COALESCE(SUM(remaining),0) AS pts
           FROM point_lots
        ...
        let totalExpired = 0;
        for (const r of rows) {
            db.prepare(`UPDATE point_lots SET remaining = 0, status = 'expired'
           WHERE member_id = ? AND status = 'active' AND remaining > 0
             AND expires_at IS NOT NULL AND expires_at <= ?`).run(r.member_id, asOf);
...
export function listLedger(memberId, limit = 50, db = getDb()) {
    return asRows(db
...
export function pointsExpiringBefore(memberId, asOf, db = getDb()) {
    const row = db
        .prepare(`SELECT COALESCE(SUM(remaining),0) AS pts FROM point_lots
        WHERE member_id = ? AND status = 'active' AND remaining > 0
          AND expires_at IS NOT NULL AND expires_at > ? AND expires_at <= ?`)
        .get(memberId, now(), asOf);
    return row.pts;
}
