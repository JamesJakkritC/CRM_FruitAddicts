...
export function ensureTiersSeeded(db = getDb()) {
    for (const t of DEFAULT_TIERS) {
        if (!getTier(t.code, db)) {
            const ts = now();
            db.prepare(`INSERT INTO membership_tiers(code, name, level, min_points, price_satang, discount_bps,
            earn_multiplier_bps, upgrade_bonus_points, duration_days, is_default, active, created_at, updated_at)
         VALUES(?,?,?,?,0,0,10000,0,NULL,?,1,?,?)`).run(t.code, t.name, t.level, t.min_points, t.is_default, ts, ts);
        }
    }
}
export function listTiers(db = getDb()) {
    return asRows(db.prepare('SELECT * FROM membership_tiers ORDER BY level ASC').all());
}
export function getTier(code, db = getDb()) {
    return asRow(db.prepare('SELECT * FROM membership_tiers WHERE code = ?').get(code));
}
export function defaultTier(db = getDb()) {
    const t = asRow(db.prepare('SELECT * FROM membership_tiers WHERE is_default = 1 ORDER BY level ASC LIMIT 1').get());
    return t ?? asRow(db.prepare('SELECT * FROM membership_tiers ORDER BY level ASC LIMIT 1').get());
}
export function defaultTierCode(db = getDb()) {
    return defaultTier(db).code;
}
export function tierDiscountBps(tierCode, db = getDb()) {
    if (!tierCode)
        return 0;
    const t = getTier(tierCode, db);
    return t && t.active ? t.discount_bps : 0;
}
export function tierEarnMultiplierBps(tierCode, db = getDb()) {
    if (!tierCode)
        return 10000;
    const t = getTier(tierCode, db);
...
export function accumulatedPoints(memberId, db = getDb()) {
    return db.prepare('SELECT COALESCE(SUM(delta),0) AS c FROM point_ledger WHERE member_id = ? AND delta > 0').get(memberId).c;
}
/**
 ...
 */
export function recomputeTier(memberId, db = getDb()) {
    ...
    if (getPolicyBool('tiers.auto_enabled', db)) {
        ...
        const t = asRow(db.prepare('SELECT level FROM membership_tiers WHERE active = 1 AND min_points <= ? ORDER BY level DESC LIMIT 1').get(acc));
        if (t)
            autoLevel = t.level;
    }
    const effLevel = Math.max(autoLevel, member.paid_tier_level ?? 0);
    const eff = asRow(db.prepare('SELECT code FROM membership_tiers WHERE active = 1 AND level <= ? ORDER BY level DESC LIMIT 1').get(effLevel));
    const code = eff?.code ?? defaultTierCode(db);
    db.prepare('UPDATE members SET tier = ?, updated_at = ? WHERE id = ?').run(code, now(), memberId);
    return code;
}
export function upsertTier(input, db = getDb()) {
    if (!input.code)
        throw badRequest('tier code required');
    const ts = now();
    db.prepare(`INSERT INTO membership_tiers(code, name, level, min_points, price_satang, discount_bps,
        earn_multiplier_bps, upgrade_bonus_points, duration_days, is_default, active, created_at, updated_at)
     VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)
     ON CONFLICT(code) DO UPDATE SET name=excluded.name, level=excluded.level, min_points=excluded.min_points,
       ...
       is_default=excluded.is_default, active=excluded.active, updated_at=excluded.updated_at`).run(input.code, input.name, input.level, input.minPoints, input.priceSatang, input.discountBps, input.earnMultiplierBps, input.upgradeBonusPoints, input.durationDays ?? null, input.isDefault ? 1 : 0, input.active === false ? 0 : 1, ts, ts);
    return getTier(input.code, db);
}
export function deleteTier(code, db = getDb()) {
    const t = getTier(code, db);
    if (!t)
        throw notFound(`tier '${code}' not found`);
    if (t.is_default)
        throw unprocessable('cannot delete the default tier');
    const inUse = db.prepare('SELECT COUNT(*) AS c FROM members WHERE tier = ?').get(code).c;
    if (inUse > 0)
        throw unprocessable(`tier '${code}' is assigned to ${inUse} member(s); reassign them first`);
...
export function upgradeMembership(args) {
    ...
    const { result } = withIdempotency('membership_upgrade', args.idempotencyKey, args, (db) => {
        const member = requireMember(args.memberId, db);
        const tier = getTier(args.tierCode, db);
        if (!tier || !tier.active)
            throw notFound(`tier '${args.tierCode}' not found or inactive`);
        if (tier.price_satang <= 0)
            throw unprocessable('this tier is not purchasable (set a price to enable paid upgrade)');
        ...
        const info = db
            .prepare(`INSERT INTO membership_purchases(member_id, tier_code, price_satang, points_granted, branch_id, actor_user_id, idempotency_key, expires_at, created_at)
         VALUES(?,?,?,?,?,?,?,?,?)`)
        ...
        if (tier.upgrade_bonus_points > 0) {
            ...
            const row = db.prepare(`SELECT COALESCE(SUM(remaining),0) AS bal FROM point_lots WHERE member_id=? AND status='active' AND remaining>0 AND (expires_at IS NULL OR expires_at>?)`).get(member.id, now());
            balance = row.bal;
        }
...
export function listMemberPurchases(memberId, db = getDb()) {
