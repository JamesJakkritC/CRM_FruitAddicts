import { openDb, getDb, now } from "./db/index.js";
import { config } from "./config.js";
import { runMigrations } from "./db/migrate.js";
import { ensureRolesSeeded, getUserByUsername, createUser } from "./domain/users.js";
 ...
 */
export function resetAdmin(username, password) {
    ...
    if (!existing) {
        ...
        db.prepare('UPDATE users SET password_hash = ?, status = ?, updated_at = ? WHERE id = ?').run(hashPassword(password), 'active', now(), existing.id);
        db.prepare("INSERT INTO user_roles(user_id, role) VALUES(?, 'super_admin') ON CONFLICT DO NOTHING").run(existing.id);
    }
    console.log('==================================================');
