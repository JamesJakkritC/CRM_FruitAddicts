...
function key() {
    ...
    if (buf.length !== 32)
        throw new Error('PII_ENCRYPTION_KEY must be base64 of exactly 32 bytes');
    return buf;
}
...
export function decrypt(stored) {
    ...
    if (!stored.startsWith(PREFIX))
        return stored; // plaintext (never encrypted)
    const k = key();
    if (!k)
        throw new Error('encrypted data present but PII_ENCRYPTION_KEY is not set');
