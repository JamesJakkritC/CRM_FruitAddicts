import { Router, clientIp } from "../lib/http.js";
import { requireLineUser } from "../lib/auth.js";
import { notFound, unauthorized } from "../lib/errors.js";
import { getDb, now } from "../db/index.js";
import { getMemberByLineUserId, registerByLineUser, } from "../domain/members.js";
import { getBalance, listLedger, redeemPoints } from "../domain/points.js";
import { listMemberTransactions, recordTransaction } from "../domain/transactions.js";
import { customer360 } from "../domain/segments.js";
import { listCoupons } from "../domain/coupons.js";
import { getOrCreateReferralCode, recordReferral, referralSummary } from "../domain/referral.js";
import { getPolicyBool, getPolicyString } from "../domain/policy.js";
import { enqueue, textMessage } from "../domain/notifications.js";
import { getStoreProfile, getLogoDataUrl, getTheme } from "../domain/store.js";
import { listBranches } from "../domain/branches.js";
import { getSignupFields } from "../domain/signup.js";
import { resolvePosKey } from "../domain/pos.js";
import { submitClaim, listMemberClaims, getClaim, getClaimImage } from "../domain/receipts.js";
import { memberItems } from "../domain/pos_import.js";
import { setConsent, exportMemberData } from "../domain/pdpa.js";
import { memberCouponIssues } from "../domain/campaigns.js";
import { audit } from "../domain/audit.js";
import { getLineProvider } from "../providers/line/index.js";
import { config } from "../config.js";
import { body, optionalString, optionalBool, optionalInt, requireInt, idempotencyKey, } from "./helpers.js";
function me(ctx) {
    const lineUserId = ctx.state.lineUserId;
    const member = getMemberByLineUserId(lineUserId);
    if (!member)
        throw notFound('Member not registered. Call POST /api/me/register first.');
    return member;
}
export function registerPublicRoutes(router) {
    ...
    router.get('/api/me/points', (ctx) => {
        const m = me(ctx);
        return { balance: getBalance(m.id), ledger: listLedger(m.id, 50) };
    }, [requireLineUser]);
    router.get('/api/me/transactions', (ctx) => ({ transactions: listMemberTransactions(me(ctx).id, 50) }), [requireLineUser]);
    router.get('/api/me/coupons', (ctx) => {
        const m = me(ctx);
        const active = listCoupons().filter((c) => {
            if (c.status !== 'active')
                return false;
            const t = now();
            if (c.starts_at && t < c.starts_at)
                return false;
            if (c.ends_at && t > c.ends_at)
                return false;
            return true;
        });
        return { coupons: active, issued: memberCouponIssues(m.id) };
    }, [requireLineUser]);
    router.post('/api/me/redeem', (ctx) => {
        const b = body(ctx);
        const m = me(ctx);
    ...
    router.get('/api/me/referral', (ctx) => {
        const m = me(ctx);
        if (!getPolicyBool('referral.enabled'))
            return { enabled: false, summary: referralSummary(m.id) };
    ...
    router.get('/api/me/receipts/:id/image', (ctx) => {
        const m = me(ctx);
        const id = Number(ctx.params.id);
        const claim = getClaim(id);
        if (!claim || claim.member_id !== m.id)
    ...
    router.post('/api/pos/transactions', (ctx) => {
        ...
        if (!resolved)
            throw unauthorized('invalid or revoked POS key');
        const b = body(ctx);
    ...
    router.post('/webhook/line', async (ctx) => {
        ...
        for (const ev of events) {
            ...
            const res = db
                .prepare(`INSERT INTO webhook_events(event_id, type, raw_json, received_at)
